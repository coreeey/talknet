...
def load_video(data, data_path, length, start, end, visual_aug):	
	...
	if len(faces.shape) == 1:
		faces = numpy.zeros((1, 112, 112))
		print('???????????????????????????????????????????')
	# 	faces = numpy.expand_dims(faces, axis=0)
...
def load_label(data, length, start, end):
	...
	return numpy.array(labels[round(start*25): round(end*25)])

class loader_TalkSet(object):
	def __init__(self, trialFileName , data_path, audio_aug=False, visual_aug=False, musanPath='', rirPath='',**kwargs):
		self.data_path = data_path
		self.audio_aug = audio_aug
		self.visual_aug = visual_aug
		self.minibatch = []
		# self.rir, self.noiselist = get_noise_list(musanPath, rirPath)
		mix_lst = open(trialFileName).read().splitlines()
		length = len(mix_lst)
		# mix_lst = mix_lst[length//5 + 100:length//4]
		# mix_lst = mix_lst[:length//5]
		...
		while True:
			...
			batch_size = int(30 / length_total)
	...
	def __getitem__(self, index):
		...
		for line in batch_lst:
			data = line.split()
			audio = load_audio(data, self.data_path, length_total, start, end, audio_aug = self.audio_aug)
			video = load_video(data, self.data_path, length_total, start, end, visual_aug = self.visual_aug)
			label = load_label(data, length_total, start, end)
			# length = int(min(audio.shape[0]//4, video.shape[0]))
			# audio = audio[:length*4, ...]
			# video = video[:length, ...]
			# label = label[:length, ...]
			audio_feature.append(audio)#, rirlist = self.rir, noiselist = self.noiselist))
			video_feature.append(video)
			labels.append(label)
